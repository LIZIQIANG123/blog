<template>
    <div class="modify">
        <!-- 修改文章或者发布新文章 -->
        <div class="head">
            <span v-show="id">修改标签</span>
            <span v-show="!id">添加标签</span>
        </div>
        <!-- 文章区域 -->
        <div class="content">
            <div class="title">
                <span>标签名：</span>
                <input type="text" v-model="passage.tagname">
            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            //获取文章原本内容
            getOneArticle(){
                api.getonetag(this.id).then(res=>{
                    switch (res.code){
                        case '200':
            ...
            savePassage(){
                ...
                if(r==true){
                    if(this.id){
                    api.updatetag(this.passage).then(res=>{
                        switch(res.code){
                            case '200':
                                alert('保存成功！');
                                this.$router.go(-1);
                     ...
                     api.addtag(this.passage).then(res=>{
                        switch(res.code){
                            case '200':
                                alert('保存成功！');
                                this.$router.go(-1);
